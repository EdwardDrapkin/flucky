...
import FluckyComponent from './FluckyComponent';
    ...
    constructor(dispatcher:FluckyDispatcher, actionCreators:Array<FluckyActionCreator>,
        ...
        for(let storeName in this.stores) {
            ...
            if(store.onError && store['onError'].constructor && store['onError'].apply && store['onError'].call) {
                let doneKey = Flucky.Dispatcher.getEventDoneKey(null, 'onError', store.constructor.name);
            ...
            for(let methodName in this.methods) {
                ...
                if(store[expectedListenerName] && store[expectedListenerName].constructor && store[expectedListenerName].apply && store[expectedListenerName].call) {
                    for(const type in this.methods[methodName]) {
                        let __eventKey = Flucky.Dispatcher.getEventKey(type, methodName);
                        let __doneKey = Flucky.Dispatcher.getEventDoneKey(null, methodName, store.constructor.name);
                ...
                for(const type in this.methods[methodName]) {
                    let _eventKey = Flucky.Dispatcher.getEventKey(type, methodName);
                    let _doneKey = Flucky.Dispatcher.getEventDoneKey(type, methodName, store.constructor.name);
