...
export default class FluckyComponent extends ReactComponent {
    ...
    shouldComponentUpdateShallow(nextProps:Object, nextState:Object) : boolean {
        return !this.shallowCompare(nextProps, nextState);
    }

    shallowCompare(nextProps:Object, nextState:Object) : boolean{
        if(nextProps === this.props && nextState === this.state) {
            return true;
        }

        let oldPropKeys = Object.keys(this.props);
        let newPropKeys = Object.keys(nextProps);

        let oldStateKeys = Object.keys(this.state);
        let newStateKeys = Object.keys(nextState);

        if(oldPropKeys.length !== newPropKeys.length ||
                oldStateKeys.length !== newStateKeys.length) {
            return false;
        }
    ...
    getStateFromStores():{[key:string]:any} {
        throw "The getStateFromStores method needs to be overloaded!";
    }
